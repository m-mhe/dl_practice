...
6.  Build a fully connected Multilayer Perceptron.
...
The main model, `modelTwo`, is a fully connected **Multilayer Perceptron
(MLP)**.
...
# 13. Explanation of the Model Architecture
...
The input layer accepts a Fashion MNIST image with dimensions 28 × 28.

Because the images are grayscale, there is only one intensity value per
pixel.
...
The Flatten layer converts the two-dimensional image into a
...
The neural network receives the image as 784 input features.
...
The first fully connected layer contains 400 neurons.

It receives the 784 flattened pixel values and learns combinations of
those input features that can contribute to distinguishing between
clothing classes.
...
The second dense layer contains 300 neurons.

It receives the representation learned by the previous layer and
attempts to learn more complex combinations of features.
...
A second 300-neuron hidden layer provides additional representational
capacity.

Stacking multiple dense layers allows the network to learn progressively
more complex transformations of the input representation.
...
This layer reduces the representation from 300 to 200 neurons while
continuing to learn useful features for classification.
...
The final hidden dense layer further processes the learned
representation before the output layer performs the final
classification.
...
after every hidden dense layer.

A dropout rate of 0.45 means that during each training step,
approximately **45% of the activations connected to that dropout layer
are randomly set to zero**.

The purpose is to reduce the model's dependence on particular neurons
and encourage it to learn more robust representations.

Dropout is a regularization technique commonly used to help reduce
overfitting.

Importantly, dropout is active during training but is automatically
disabled during normal inference/evaluation. During evaluation, the full
network is used.

The repeated use of a relatively high dropout rate in this model was
part of the project's effort to control overfitting despite the model's
